...
makeCacheMatrix <- function(x = matrix()) {
  ...
  setInverse <-
    function(solve)
      m <<- solve ## set the inverse matrix
  getInverse <- function()
    m ## get the inverse matrix
  list(
...
cacheSolve <- function(x, ...) {
  ...
  m <- x$getInverse() ## query the matrix's cache
  if (!is.null(m)) {
    ## if there is a cache the inverse has been previously calculated
  ...
  data <-
    x$get()  ## get the matrix used by makeCacheMatrix function
